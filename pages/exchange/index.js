  ...
  useEffect(() => {
    ...
    const fetchExchangeRates = async () => {
      await fetch("/latest.json")
        ...
        .then((data) => {
          console.log(`DATA: ${data["INR"]}`);
          const generateRandom = () => 1 + Math.random() / 100;
          console.log(generateRandom());
          Object.keys(data).forEach((key) => {
            data[key] *= generateRandom();
          });
          console.log(`NEW DATA: ${data["INR"]}`);
                ...
                <span className="rate">
                  {selectedCountry && amount
                    ? (
                        ...
                        exchangeRates[selectedCountry.code] * amount * rate.rate
